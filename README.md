选题背景与研究现状
1.1 选题背景
导数作为微积分学的核心概念，不仅是数学分析、高等数学课程的教学核心，更在物理学、工程计算、经济学建模等多个领域有着广泛且关键的实际应用。函数的单调性与极值是刻画函数性态的核心特征，也是解决各类最优化问题的重要理论基础。在传统教材体系与教学实践中，普遍采用求导公式结合一阶、二阶导数判别法研究函数的单调性与极值，该方法在显函数可导且导函数易于求解的场景下高效且实用。但这类方法的应用依赖于函数在某区间内可导、导函数易于计算的前提条件，当面对分段函数在分段点处导数不存在、隐函数难以显化求导等特殊情形时，常规判别法往往因前提条件不满足而失效，导致函数性态分析出现理论与实践的盲区。针对分段函数在分段点处的可导性问题，王银坤、倪谷炎与张守江等学者进行了系统研究[1][12]；针对隐函数的求导与极值判定，杨雄、袁新全与李文东等也提出了相应的方法探索[3][13]。这些研究虽已触及特殊函数的分析难点，但多聚焦于具体方法层面，从导数定义出发的系统性拓展研究仍有待深入。因此，回归导数定义这一基本概念，系统探讨其在分段函数、隐函数这类特殊函数的单调性与极值判定中的应用，构建适配本科数学教学与常规问题求解的判定方法，具有重要的理论补充意义与教学实践价值。
1.2 研究现状
函数单调性与极值的判定，始终是微积分学研究和本科数学教学的重点内容，相关领域的研究已形成较为完善的理论体系。国内外经典教材均围绕导数公式构建了系统的函数性态判别方法，如同济大学数学系编写的《高等数学》、华东师范大学数学系编写的《数学分析》等，系统阐述了基于导函数符号的单调性与极值判别法，成为本科阶段函数分析的通用基础方法。国外经典教材如Rudin的《数学分析原理》[10]、Spivak的《微积分》[9]则更注重导数定义的本质内涵与极限思想的阐释，强调从概念本源出发分析函数性质，为导数定义的拓展应用提供了理论支撑。在现代分析学领域，针对分段函数、隐函数这类非光滑函数的分析，学者们发展出次微分、广义导数等拓展概念，如阮诗佺等对广义导数及其在极值问题中的应用进行了探讨[15]，其理论根基正是对经典导数定义的延伸与深化，也印证了从导数定义出发分析特殊函数性态的必要性。
在上述理论框架下，国内外学者针对导数应用与函数性态判定的教学、解题难点展开了诸多细化研究。在分段函数方面，裴礼文[8]在《数学分析中的典型问题与方法》中列举了多种利用导数定义讨论分段函数可导性的经典范例，明确了定义法在分段函数基础分析中的核心地位；张守江[1]、卢刚夫[11]等具体研究了分段点处的导数求解方法；赵邦杰、郭瑞海[7]对分段函数在分段点的极限、连续、可导性进行了系统研究；王银坤、倪谷炎[12]进一步提出了分段函数在分段点处可导性的判别方法，为分段函数的性态分析提供了具体思路。在隐函数研究方面，冯秀红[6]、郎开禄[4]探讨了隐函数的极值求法与一元隐函数取极值的一般充分条件；杨雄、袁新全[3]对隐函数求导方法进行了优化探索；李文东[13]则将隐函数求导应用于极值点偏移问题的研究，拓展了隐函数极值问题的研究视角。此外，陈思维[14]探讨了利用导数研究含参函数的单调性与极值的方法；李军成等[2]从计算数学角度研究了导数振荡与应变能极小的平面分段三次参数Hermite插值问题，为分段函数的应用研究提供了新视角。在教学层面，宋莉莉、王继宏[5]围绕导数教学提出回归定义教学以深化学生对导数本质的理解，为导数定义在教学中的渗透提供了思路。
现有研究在导数理论体系构建、常规函数性态判定、特殊函数基础分析及教学实践等方面均取得了丰硕成果，为分段函数、隐函数的单调性与极值判定提供了重要的理论参考与方法支撑，但还有分段函数分段点不可导、隐函数显化困难等具体问题，这也为本次研究提供了进一步探索与完善的空间，即通过回归导数的定义，结合极限局部保号性，系统探讨其在判定函数单调性与极值中的应用。
参考文献

[1]张守江.摭谈分段函数在分段点的导数[J].高等数学研究,2021,24(05):14-17.
[2]李军成,刘成志,郭啸.导数振荡与应变能极小的平面分段三次参数Hermite插值[J].计算数学,2022,44(01):97-106.
[3]杨雄,袁新全.隐函数求导方法探索[J].玉溪师范学院学报,2023,39(06):21-26.
[4]郎开禄.一元隐函数取极值的一般充分条件[J].高等数学研究,2017,20(01):69-71+75.
[5]宋莉莉,王继宏.关于导数的教学思考[J].高等数学研究,2025,28(05):67-70+95.
[6]冯秀红.隐函数的极值求法[J].高师理科学刊,2010,30(03):17-19.
[7]赵邦杰,郭瑞海.对分段函数在分段点的极限、连续、可导性的研究[J].西南民族大学学报（自然科学版）,2003,29(4):402-405
[8]裴礼文. 数学分析中的典型问题与方法[M]. 2版. 北京: 高等教育出版社, 2006.
[9]Spivak M. Calculus[M]. 4th ed. Houston: Publish or Perish, 2008.
[10]Rudin W. Principles of Mathematical Analysis[M]. 3rd ed. New York: McGraw-Hill, 1976.
[11]卢刚夫.确定分段函数导数的方法[J].高师理科学刊,2017,37(08):86-87+99.
[12]王银坤,倪谷炎.分段函数在分段点处可导性的判别方法[J].大学数学,2023,39(01):67-71.
[13]李文东.隐函数求导在极值点偏移问题中的应用[J].中学数学研究(华南师范大学版),2024,(03):6-9.
[14]陈思维.利用导数研究含参函数的单调性与极值[J].中学数学,2024,(15):72-73.
[15]阮诗佺,于小珊,于野.广义导数及其在极值问题中的应用[J].大学数学,2024,40(02):73-80.
